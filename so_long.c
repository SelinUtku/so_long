...
/*   Updated: 2023/03/05 06:33:05 by sutku            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	enemy_move(t_game *game, int i, int j, int sec)
{
	...
	if (game->game_state == 0)
	{
		if (game->spd == 1)
		{
			if (sec % 30 == 0)
			{
				if (game->map_arr[i + incr][j] == '1' || game->map_arr[i + incr][j] == 'C' || game->map_arr[i + incr][j] == 'E')
					incr *= -1;
...
int32_t	main(int argc, char **argv)
{
	...
	mlx_loop(game.mlx);
	// delete_images(game.mlx, &img);
	// delete_strings(game.mlx, &str, &game);
