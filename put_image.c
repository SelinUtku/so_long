...
/*   Updated: 2023/03/05 06:46:03 by sutku            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_assets_to_map(mlx_t *mlx, t_game *game, t_images *img)
{
	...
	{
		if (mlx_image_to_window(game -> mlx, game->imgs->rip_img, 0, 100) < 0)
			error_message(MLX_IMG_WND, game);
		return ;
	}
	if (game->game_state == 1)
	{
		if (mlx_image_to_window(game -> mlx, game->imgs->win_img, 0, 100) < 0)
			error_message(MLX_IMG_WND, game);
		return ;
	}
	i = -1;
